...
router.post("/", (request, response, next) => {
    ...
    let insert = `INSERT INTO Messages(ChatId, Message, MemberId)
                  VALUES($1, $2, $3) 
                  RETURNING MessageId AS MessageId, ChatId, Message, MemberId AS email, TimeStamp`
...
router.get("/:chatId?/:messageId?", (request, response, next) => {
        ...
        let query = `SELECT Messages.messageId AS messageId, Members.Email, CONCAT(Members.firstname, ' ', Members.lastname) as firstname, Messages.Message, 
                    to_char(Messages.Timestamp AT TIME ZONE 'PDT', 'YYYY-MM-DD HH24:MI:SS.US' ) AS Timestamp
                    FROM Messages
                    INNER JOIN Members ON Messages.MemberId=Members.MemberId
                    WHERE ChatId=$1 AND Messages.messageId < $2
