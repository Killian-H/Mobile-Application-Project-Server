...
/**
 * @api {get} / Request to get chats user in
 * @apiName GetChats
 * @apiGroup Chats
 * 
 ...
 * @apiSuccess (Success 201) {Number} chatList all chats user is in
 * 
 * @apiError (404: Unknown user) {String} message "member is not in any chats"
 * 
        ...
        .then(result => {
            if (result.rowCount == 0) {
                response.status(404).send({
                    message: "member is not in any chats"
                })
            } else {
                response.send({
                    success: true,
 ...
 */ 
 router.post("/", (request, response, next) => {
    ...
    let insertChatMember = `INSERT INTO ChatMembers(ChatId, MemberId)
                  VALUES ($1, $2)
                  RETURNING *`
    let chatMemberValues = [response.locals.chatId, request.decoded.memberid]
    pool.query(insertChatMember, chatMemberValues)
        .then(result => {
            response.send({
                success: true,
                chatID:chatMemberValues[0]
            })
        }).catch(err => {
            response.status(400).send({
                message: "SQL Error",
...
router.put("/:chatId/", (request, response, next) => {
    ...
    if (!request.params.chatId || !request.body.memberid || isNaN(request.body.memberid)) {
        response.status(400).send({
            message: "Missing required information"
        })
